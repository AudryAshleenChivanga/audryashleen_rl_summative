...
log_dir = "logs/ppo_improved/"
model_dir = "models/ppo_improved/"
...
while step < total_timesteps:
    ...
    try:
        with open(os.path.join(log_dir, "evaluations.npz"), "rb") as f:
            data = np.load(f)
            if "results" in data:
                last_eval_reward = float(np.mean(data["results"][-1]))
    except:
        last_eval_reward = -float("inf")

    curriculum.advance_if_ready(last_eval_reward)
    env = DummyVecEnv([make_env(curriculum.current_level())])
    model.set_env(env)
    eval_callback.eval_env = DummyVecEnv([make_env(curriculum.current_level())])

model.save(os.path.join(model_dir, "ppo_gi2d_improved"))
